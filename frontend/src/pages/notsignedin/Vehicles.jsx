...
function Vehicles() {
  ...
  const uniqueTypes = [
    ...new Set(vehicles.map((vehicle) => vehicle.type || "Unknown")),
  ];
  ...
  useEffect(() => {
    ...
    if (selectedTypes.length > 0) {
      filtered = filtered.filter((vehicle) =>
        selectedTypes.includes(vehicle.type || "Unknown")
      );
    }
  ...
  const getActiveFiltersCount = () => {
    return (
      selectedBrands.length + selectedTypes.length + selectedPriceRanges.length
    );
  };
    ...
    >
      ...
      >
        <div style={{ padding: "0 0px", width: "100%" }}>
          ...
          >
            Browse our complete collection of vehicles from all major
            manufacturers.
          </p>
        </div>
      </section>
