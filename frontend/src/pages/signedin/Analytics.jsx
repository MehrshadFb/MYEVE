...
function Analytics() {
  ...
  const downloadVehicleAnalytics = () => {
    try {
      ...
      const totalInventory = vehicles.reduce((sum, vehicle) => sum + (vehicle.quantity || 0), 0);
      const totalValue = vehicles.reduce((sum, vehicle) => sum + ((vehicle.quantity || 0) * (vehicle.price || 0)), 0);
      const timesOrdered = vehicles.reduce((sum, vehicle) => sum + (vehicle.amountSold || 0), 0);
      ...
      csvContent += "Brand,Vehicle Count,Total Inventory,Total Value,Times Ordered\n";
      ...
      csvContent += "Brand,Model,Year,Quantity,Price,Times Ordered,Total Value\n";
      ...
      link.setAttribute("download", `vehicle-analytics-${new Date().toISOString().split('T')[0]}.csv`);
      link.style.visibility = 'hidden';
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);

      setMessage({ type: "success", text: "Vehicle analytics downloaded successfully!" });
    } catch (error) {
      console.error("Error downloading vehicle analytics:", error);
      setMessage({ type: "error", text: "Failed to download vehicle analytics" });
    }
  };
  ...
  const downloadUserAnalytics = () => {
    try {
      ...
      setMessage({ type: "success", text: "User analytics downloaded successfully!" });
    } catch (error) {
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: "1.2rem", color: "#64748b" }}>Loading analytics data...</div>
        </div>
      </div>
    );
  }
            ...
            <div style={{ marginBottom: "20px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                <div style={{ textAlign: "center", padding: "12px", backgroundColor: "#f8fafc", borderRadius: "8px" }}>
                  <div style={{ fontSize: "1.5rem", fontWeight: "700", color: "#3b82f6" }}>{totalVehicles}</div>
                  <div style={{ fontSize: "0.85rem", color: "#64748b" }}>Total Vehicles</div>
                </div>
                <div style={{ textAlign: "center", padding: "12px", backgroundColor: "#f8fafc", borderRadius: "8px" }}>
                  <div style={{ fontSize: "1.5rem", fontWeight: "700", color: "#059669" }}>{totalInventory}</div>
                  <div style={{ fontSize: "0.85rem", color: "#64748b" }}>Total Inventory</div>
                </div>
                <div style={{ textAlign: "center", padding: "12px", backgroundColor: "#f8fafc", borderRadius: "8px" }}>
                  <div style={{ fontSize: "1.2rem", fontWeight: "700", color: "#dc2626" }}>${totalValue.toLocaleString()}</div>
                  <div style={{ fontSize: "0.85rem", color: "#64748b" }}>Total Value</div>
                </div>
                <div style={{ textAlign: "center", padding: "12px", backgroundColor: "#f8fafc", borderRadius: "8px" }}>
                  <div style={{ fontSize: "1.5rem", fontWeight: "700", color: "#8b5cf6" }}>{timesOrdered}</div>
                  <div style={{ fontSize: "0.85rem", color: "#64748b" }}>Times Ordered</div>
                </div>
              </div>
            </div>
            ...
            <div style={{ marginBottom: "20px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "12px" }}>
                <div style={{ textAlign: "center", padding: "16px", backgroundColor: "#f8fafc", borderRadius: "8px" }}>
                  <div style={{ fontSize: "2rem", fontWeight: "700", color: "#3b82f6" }}>{totalUsers}</div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                  <div style={{ textAlign: "center", padding: "12px", backgroundColor: "#fee2e2", borderRadius: "8px" }}>
                    <div style={{ fontSize: "1.5rem", fontWeight: "700", color: "#dc2626" }}>{adminCount}</div>
                    <div style={{ fontSize: "0.85rem", color: "#64748b" }}>Admins</div>
                  </div>
                  <div style={{ textAlign: "center", padding: "12px", backgroundColor: "#dcfce7", borderRadius: "8px" }}>
                    <div style={{ fontSize: "1.5rem", fontWeight: "700", color: "#059669" }}>{customerCount}</div>
                    <div style={{ fontSize: "0.85rem", color: "#64748b" }}>Customers</div>
                  </div>
                </div>
              </div>
            </div>
